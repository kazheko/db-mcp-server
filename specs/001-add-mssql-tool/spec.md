...
An LLM client connected through the MCP protocol invokes the MSSQL query tool with database name, SQL text, and an optional row limit so the adapter stub can return a representative recordset along with correlation and timing metadata.
...
1. **Given** the MCP server is running and tooling metadata is registered, **When** the LLM invokes the MSSQL tool with `database="hr"`, `query="SELECT * FROM employees"`, and `maxRows=5`, **Then** the server returns a payload containing `correlationId`, the echoed database name, a stubbed recordset capped at five rows, and populated `startedAt`/`completedAt` values.
2. **Given** the adapter stub can vary its payload size, **When** the tool is invoked repeatedly with different `maxRows` values, **Then** the handler forwards that value to the adapter and the resulting recordset never exceeds the requested limit.
...
1. **Given** the MCP handshake is performed, **When** the client enumerates registered tools, **Then** the MSSQL tool entry presents the configured title/description plus input fields (`database`, `query`, `maxRows`) and output fields (`correlationId`, `database`, `recordset`, `startedAt`, `completedAt`).
...
- Adapter returns an empty recordset; the response still includes `correlationId`, timestamps, and an empty `recordset` array to signal no rows were available.
...
- **FR-006**: The MSSQL adapter MUST be implemented as a placeholder stub that returns deterministic mock recordsets and explicitly avoids connecting to any database or requiring credentials.
- **FR-007**: Successful tool responses MUST echo the requested `database`, include the adapter-provided `recordset`, and honor any `maxRows` limit when building the data.
...
- **MSSQL Adapter Stub**: A placeholder component that accepts database/query/maxRows, returns mock recordsets, or throws errors to simulate failure without touching a real database.
- **Tool Response Envelope**: Standard structure returned to LLM clients with `correlationId`, `database`, `recordset`, `startedAt`, and `completedAt`, as well as propagated errors when execution fails.
...
- **SC-003**: Across at least 10 sample invocations with varying `maxRows` values, every returned `recordset` respects the requested limit and includes mock rows shaped consistently.
