...
**Input**: Design documents from `C:\home\ykazheka\db-mcp-server\specs\001-mssql-adapter\`
...
- [X] T001 Add the `mssql` runtime dependency to `C:\home\ykazheka\db-mcp-server\package.json` and regenerate `C:\home\ykazheka\db-mcp-server\package-lock.json` so the driver is installable everywhere.
- [X] T002 Create a checked-in `C:\home\ykazheka\db-mcp-server\.env.example` documenting the `MSSQL_CONNECTION_STRING` variable so local environments can be bootstrapped consistently.
...
- [X] T003 Implement `loadConnectionConfig()` in `C:\home\ykazheka\db-mcp-server\src\adapters\mssql-config.ts` to read `MSSQL_CONNECTION_STRING`, trim it, and expose a typed `MssqlConnectionConfig` object shared by all adapters.
- [X] T004 Extend `C:\home\ykazheka\db-mcp-server\src\types\mssql.ts` with the `MssqlConnectionConfig`, `MssqlMetadataQuery`, and `MssqlQueryResult` interfaces referenced in the data model so adapters and tools share the same contracts.
- [X] T005 [P] Add a reusable mock for the `mssql` driver under `C:\home\ykazheka\db-mcp-server\tests\unit\adapters\__mocks__\mssql.ts` (plus a helper index) so later Vitest suites can simulate connection pools without a live server.
...
- [X] T006 [US1] Rewrite `C:\home\ykazheka\db-mcp-server\src\adapters\mssql.ts` to instantiate a singleton `mssql.ConnectionPool`, execute the provided SQL, cap returned rows to `maxRows`, and propagate driver errors untouched.
- [X] T007 [US1] Update `C:\home\ykazheka\db-mcp-server\src\server\index.ts` so it constructs the new adapter via `loadConnectionConfig()` instead of `StubMssqlAdapter`, ensuring only the production adapter is registered with the MCP server.
- [X] T008 [P] [US1] Refresh `C:\home\ykazheka\db-mcp-server\src\tools\mssql-tool.ts` and `C:\home\ykazheka\db-mcp-server\src\tools\tool-factory.ts` copy to describe the real adapter (no “deterministic stub”) and confirm the handler still emits `correlationId`, timing fields, and JSON rows.
- [X] T009 [US1] Expand `C:\home\ykazheka\db-mcp-server\tests\unit\adapters\mssql.test.ts` to mock the driver, covering success rows, row-capping, and error propagation from `mssql.Request.execute`.
- [X] T010 [US1] Update `C:\home\ykazheka\db-mcp-server\tests\contract\mssql-tool.contract.test.ts` to assert the MCP tool streams the adapter payload verbatim (content + errors) instead of the old synthetic rows.
...
- [X] T011 [US2] Enhance `C:\home\ykazheka\db-mcp-server\src\adapters\mssql-config.ts` with defensive checks (empty string, malformed prefix) that throw descriptive startup errors before any pool connects.
- [X] T012 [P] [US2] Add Vitest coverage to `C:\home\ykazheka\db-mcp-server\tests\unit\adapters\mssql-config.test.ts` proving missing or malformed environment variables cause the adapter bootstrap to reject.
- [X] T013 [P] [US2] Update `C:\home\ykazheka\db-mcp-server\specs\001-mssql-adapter\quickstart.md` (and reference it from `.env.example`) so operators know how to set, rotate, and restart after changing `MSSQL_CONNECTION_STRING`.
...
- [X] T014 [US3] Delete the stub-specific templates, constants, and `StubMssqlAdapter` export from `C:\home\ykazheka\db-mcp-server\src\adapters\mssql.ts`, leaving only the real adapter implementation.
- [X] T015 [P] [US3] Remove any lingering `StubMssqlAdapter` imports/usages from `C:\home\ykazheka\db-mcp-server\src\server\index.ts`, `C:\home\ykazheka\db-mcp-server\tests`, and `C:\home\ykazheka\db-mcp-server\scripts\mcp\invoke.ts` so build artifacts can’t reference the stub.
- [X] T016 [P] [US3] Update the manual notes in `C:\home\ykazheka\db-mcp-server\AGENTS.md` and related docs to drop references to “deterministic stub” now that only the real adapter remains.
...
- [X] T017 [P] Refresh `C:\home\ykazheka\db-mcp-server\specs\001-mssql-adapter\contracts\mssql-query.openapi.yaml` so descriptions match the real adapter behavior (no stub wording, no premature SQL validation claims).
- [X] T018 Run `npm run build` and `npm test` from `C:\home\ykazheka\db-mcp-server\` to ensure TypeScript outputs and Vitest suites pass with the new driver on CI.
